...
class Login
{
	...
	function Content()
	{
		if(isset($_POST['login']))
		{
			...
			{
				$res = mysql_query("SELECT acct,reward_points FROM accounts WHERE login = '{$Account}' AND encrypted_password = SHA(CONCAT(UPPER({$Account}),\":\",UPPER({$Password})))");
			}
			else
			{
			...
			$_SESSION['vcp']['points'] = $Row['reward_points'] > 0 ? (int)$Row['reward_points'] : 0;
			//success
			$this->Success();
			return;
		}
